 ...
 * - Détecte et ferme les clients inactifs (timeout 10s)
 ...
 * - Log chaque événement important (ACCEPT, RECV, 404, TIMEOUT, CLOSE...)
 *
 * Utilisation :
 *   1. make lvl9 && ./main /chemin/absolu/fichier.html
 *   2. curl http://localhost:8080/ ou ouvrir dans un navigateur
 *
 * Le serveur ne gère que les requêtes GET simples, sans keep-alive.
 **********************************************************************/
...
struct Client {
	...
	Client(int fd_) : fd(fd_), file_fd(-1), file_offset(0), file_size(0), sending(false), last_activity(time(NULL)) {}
};
...
void run_server_loop(int server_fd, struct sockaddr_in *address, char *default_file) {
	...
	while(!stop_flag){
		const int TIMEOUT = 10; // secondes d'inactivité max
		time_t now = time(NULL);
		for (int i = 1; i < (int)fds.size(); i++) {
			Client &cli = clients[i];
			if (now - cli.last_activity > TIMEOUT) {
				log_event(cli.fd, "TIMEOUT", 0);
				close(cli.fd);
				log_event(cli.fd, "CLOSE", 0);
